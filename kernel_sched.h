...
/**
  ...
  An object of this type is associated to every thread. 
*/

typedef struct process_thread_control_block { /** the thing we must make **/
	...
	int exited;	// wether thread is exited
	int detached;	// wether thread is detadhed
	
	CondVar exit_cv;
	int refcount;	// how many times it is referenced
